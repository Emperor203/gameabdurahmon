...
// Коллбэки для связи с React
let updateScoreUpdateFn;
let updateGameStateFn;

export function initGame(container, setScore, setGameState) {
  updateScoreUpdateFn = setScore;
  updateGameStateFn = setGameState;
  ...
  // Очищаем контейнер перед добавлением, чтобы не дублировать канвас
  container.innerHTML = '';
...
export function startGame() {
  ...
  if (updateScoreUpdateFn) updateScoreUpdateFn(0);
...
function splitBlockAndAddNextOneIfOverlaps() {
  ...
  if (overlap > 0) {
    ...
    if (updateScoreUpdateFn) updateScoreUpdateFn(stack.length - 1);
...
function missedTheSpot() {
  ...
  gameEnded = true;
  if (updateGameStateFn && !autopilot) updateGameStateFn('ENDED');
}
